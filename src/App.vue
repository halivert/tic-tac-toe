		...
		<section class="place-self-center flex flex-col gap-3 relative">
			...
			>
				<div class="bg-white py-4 px-8 rounded-sm flex gap-4 flex-col">
					...
					<button
						class="px-2 py-1 bg-zinc-700 rounded text-white"
						@click="restart"
					>
						Reiniciar
					</button>
			...
			<div class="flex gap-3" v-for="(row, i) in board" :key="i">
				<button
					:class="[
						...
						{ 'bg-red-300': cell === X && partOfWinningCoordinates(i, j) },
						{ 'bg-teal-300': cell === O && partOfWinningCoordinates(i, j) },
						{
							'focus:bg-red-50 hover:bg-red-50 accent-red-800':
								turn === X && !cell,
						},
						{
							'focus:bg-teal-50 hover:bg-teal-50 accent-teal-800':
								turn === O && !cell,
						},
						{ 'bg-zinc-200': !cell || winner },
					]"
					v-for="(cell, j) in row"
					:key="`${i},${j}`"
